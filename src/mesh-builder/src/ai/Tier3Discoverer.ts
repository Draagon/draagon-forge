...
/**
 * Enriched context for Tier 3 discovery with full project knowledge.
 * Generated by ExtractionContextProvider (REQ-034).
 */
export interface EnrichedTier3DiscoveryContext extends Tier3Context {
...
export interface Tier3DiscovererConfig {
  ...
  /** Maximum number of files to batch together (cost optimization) */
  batchSize: number;
...
export class Tier3Discoverer {
   ...
   * This is the basic discovery method without enriched context.
   */
  ...
  /**
   * Discover code structure using AI with enriched context.
   * This method uses the full project context from ExtractionContextProvider.
   */
  async discoverWithContext(context: EnrichedTier3DiscoveryContext): Promise<Tier3Result> {
    // Skip files that are too large
    if (context.file.size > this.config.maxFileSize) {
      return this.createEmptyResult(context, 'File too large for AI discovery');
    }
    ...
    // Call AI for discovery with enriched context
    const aiResult = await this.aiClient.discoverWithContext({
      file: context.file.relativePath,
      language: context.file.language,
      content,
    ...
    // Calculate confidence (boost if we had good context)
    let avgConfidence =
      nodes.length > 0
        ? nodes.reduce((sum, n) => sum + n.extraction.confidence, 0) / nodes.length
        : 0;

    // Boost confidence if we had framework detection from context
    if (context.frameworks && context.frameworks.length > 0) {
      const frameworkBoost = context.frameworks[0]!.confidence * 0.1;
      avgConfidence = Math.min(1.0, avgConfidence + frameworkBoost);
    }
    ...
    return {
      ...
      frameworkDetected: aiResult.data.frameworkDetected || (context.frameworks?.[0] ? {
        name: context.frameworks[0].name,
        confidence: context.frameworks[0].confidence,
      } : undefined),
      schemaSuggestions,
      tokensUsed: aiResult.tokensUsed,
      aiCalls: 1,
  ...
  /**
   * Discover code structure for multiple files with batching.
   * More cost-efficient than calling discover() for each file.
   */
  async discoverBatch(contexts: Tier3Context[]): Promise<Tier3Result[]> {
    if (contexts.length === 0) {
      return [];
    }

    // Filter out files that are too large
    const validContexts: Array<{ context: Tier3Context; index: number }> = [];
    const results: Tier3Result[] = new Array(contexts.length);

    for (let i = 0; i < contexts.length; i++) {
      const context = contexts[i]!;
      if (context.file.size > this.config.maxFileSize) {
        results[i] = this.createEmptyResult(context, 'File too large for AI discovery');
      } else {
    ...
    for (let batchStart = 0; batchStart < validContexts.length; batchStart += batchSize) {
      const batch = validContexts.slice(batchStart, batchStart + batchSize);

      // Process batch concurrently
      const batchPromises = batch.map(async ({ context, index }) => {
        const result = await this.discover(context);
      ...
      if (batchStart + batchSize < validContexts.length && this.config.batchDelayMs > 0) {
  ...
  /**
   * Estimate cost for discovering a set of files.
   * Returns estimated tokens that will be used.
   */
  estimateCost(contexts: Tier3Context[]): {
    totalFiles: number;
    ...
    for (const context of contexts) {
      if (context.file.size > this.config.maxFileSize) {
        ...
        processable++;
        // Estimate tokens: ~1 token per 4 chars for input + ~2000 for output
        const inputTokens = Math.min(context.file.size / 4, this.config.maxPromptTokens);
        estimatedTokens += inputTokens + 2000; // output estimate
      }
    }
